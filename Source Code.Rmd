...
ggbarplot(gdna, x="ID2", y="Log10_Value", add=c("mean_sd", "point"), fill="ID2", palette=c("white", "darkseagreen1", "darkseagreen2", "darkseagreen3", "darkseagreen", "darkseagreen4")) + 
     ...
     rremove("legend") + 
     theme(axis.text.x=element_text(angle=45, hjust=1)) + 
     xlab("Mizoribine Concentration (µM)") +
...
ggbarplot(x, x="ID2", y="log2", add=c("mean_sd", "point"), color="black", fill="ID2", palette=c("white", "chartreuse3"), facet.by="Gene2", ncol=6) +
     scale_y_continuous(limits = c(-3.5, 3), expand=expansion(mult=c(0.1, 0.1))) + 
     ylab(label="Log2 Transcript Expression") + 
     rremove("legend") + 
     theme(axis.text.x=element_text(angle=45, hjust=1)) + 
     rremove("xlab") +
     theme(strip.text = element_text(face = "italic")) +
     stat_compare_means(comparisons=list(c("UTD24", "MIZ24")), label="p.signif", method="t.test", label.y=c(2)) +
     geom_hline(yintercept = 0.0) +
...
ggbarplot(x, x="Condition2", y="Normalized_Value", add=c("mean_sd", "point"), color="black", fill="Condition2", palette=c("darkolivegreen1", "darkolivegreen"), position = position_dodge(), facet.by = "Treatment2", ncol=2) +
     scale_y_continuous(limits = c(0, 700000), expand=expansion(mult=c(0,0.1))) +
...
ggbarplot(x, x="ID2", y="log2", add=c("mean_sd", "point"), color="black", fill="ID2", palette=c("white", "chartreuse3"), facet.by="Gene2", ncol=2) +  
     scale_y_continuous(limits = c(-1, 2), expand=expansion(mult=c(0.1, 0.1))) + 
     ylab(label="Log2 Transcript Expression") + 
     rremove("legend") + 
     theme(axis.text.x=element_text(angle=45, hjust=1)) + 
     rremove("xlab") +
     theme(strip.text = element_text(face = "italic")) +
     stat_compare_means(comparisons=list(c("UTD24", "MIZ24")), label="p.signif", method="t.test", label.y=c(1)) +
  geom_hline(yintercept = 0.0) +
  theme(text = element_text(size = 10)) 
```
...
```{r}
#FIGURE 8
setwd("~/Documents/Paper 3/PNAS/Monisha's Data")
library(ggplot2)
library(ggpubr)

#Figure 8B
size<-read.csv("BPD_MIZ_Random.csv")
size$Treatment2<-factor(size$Treatment, levels=c("UTD", "BPD", "MIZ", "BPD+MIZ"))

kruskal.test(Treatment~Size, data = size)
pairwise.wilcox.test(size$Size, size$Treatment, p.adj="bonferroni")

p<-ggviolin(size, x="Treatment2", y="Size", add='median_mad', fill="Treatment2", palette=c("white", "dodgerblue1", "dodgerblue3", "dodgerblue4")) +
  labs(y = bquote('Inclusion Size'~(µm^2))) +
  rremove('xlab') +
  scale_y_continuous(expand=expansion(mult=c(0.1,0.1))) +
  rremove('legend') +
  theme(axis.text.x=element_text(angle=45, hjust=1)) +
  theme(line=element_line(size=1, colour="black"), panel.border=element_rect(colour="black", fill="NA", size=1)) +
  geom_signif(comparisons=list(c("UTD", "BPD"), c("UTD", "MIZ"), c("UTD", "BPD+MIZ"), c("BPD", "BPD+MIZ"), c("MIZ", "BPD+MIZ")), annotation=c("ns", "ns", "****", "****", "****"), y_position=c(2.25, 2.5, 2.75, 3., 3.25))

ggexport(p, filename = "BPD_MIZ_random_Inclusion size_violin_3x5.5_08212022.pdf", width = 3, height = 5.5)

#Figure 8C
gdna<-read.csv("MIZ_gDNA_qPCR_08212022.csv")
gdna$ID2<-factor(gdna$ID, levels=c("UTD", "BPD", "MIZ", "BPD+MIZ"))

gdnaaov<-aov(Value~ID2, data=gdna)
summary(gdnaaov)
TukeyHSD(x=gdnaaov)

p<-ggbarplot(gdna, x="ID2", y="Log10_Value", add=c("mean_sd", "point"), fill="ID2", palette=c("white", "dodgerblue1", "dodgerblue3", "dodgerblue4")) + 
     scale_y_continuous(expand=expand_scale(mult=c(0,0.1))) + 
     ylab(label="Log10 Genome Equivalents/ng gDNA") + 
     rremove("legend") + 
     theme(axis.text.x=element_text(angle=45, hjust=1)) + 
...
ggexport(p, filename = "BPD_MIZ_gDNA_ggpubr_3x5.5_08212022.pdf", width = 3, height = 5.5)

#Figure 8D
x<-read.csv("MIZ_IFU_08212022.csv")
x$ID2<-factor(x$ID, levels=c("UTD", "BPD", "MIZ", "BPD+MIZ"))

ifuaov<-aov(log_IFU_mL~ID2, data=x)
summary(ifuaov)
TukeyHSD(x=ifuaov)

p<-ggbarplot(x, x="ID2", y="log_IFU_mL", add=c("mean_sd", "point"), fill="ID2", palette=c("white", "dodgerblue1", "dodgerblue3", "dodgerblue4")) + 
     scale_y_continuous(expand=expansion(mult=c(0,0.1))) + 
     ylab(label="Log10 IFU/mL") + 
     rremove("legend") + 
     theme(axis.text.x=element_text(angle=45, hjust=1)) + 
     rremove("xlab") +
     theme(line=element_line(size=1, colour="black"), panel.border=element_rect(colour="black", fill="NA", size=1)) +
  geom_signif(comparisons=list(c("UTD", "BPD"), c("UTD", "MIZ"), c("UTD", "BPD+MIZ"), c("BPD", "BPD+MIZ"), c("MIZ", "BPD+MIZ")), annotation=c("ns", "ns", "*", "*", "*"), y_position=c(8.25, 9, 9.75, 10.5, 11.25)) +
  geom_hline(yintercept = 3.976781123, linetype = 2)

ggexport(p, filename = "BPD_MIZ_IFU_ggpubr_3x5.5_08212022.pdf", width = 3, height = 5.5)
...
library(ggplot2)
library(ggpubr)
x<-read.csv("Ctr_RTqPCR_unique_geom5_09042022.csv")
x$Gene2<-factor(x$Gene, levels=c("amiA", "tyrP", "incF", "incG", "ispE", "sodM"))
x$Treatment2<-factor(x$Treatment, levels=c("Mock", "Bpd", "Trp"))
x$ID2<-factor(x$ID, levels=c("UTD24", "BPD16", "TRP16", "BPD24", "TRP24"))

amiA<-subset(x, Gene%in%c("amiA"))
tyrP<-subset(x, Gene%in%c("tyrP"))
incF<-subset(x, Gene%in%c("incF"))
incG<-subset(x, Gene%in%c("incG"))
ispE<-subset(x, Gene%in%c("ispE"))
sodM<-subset(x, Gene%in%c("sodM"))
...
incFaov<-aov(log2~ID, data=incF)
summary(incFaov)
TukeyHSD(x=incFaov)

incGaov<-aov(log2~ID, data=incG)
summary(incGaov)
TukeyHSD(x=incGaov)

ispEaov<-aov(log2~ID, data=ispE)
summary(ispEaov)
TukeyHSD(x=ispEaov)

sodMaov<-aov(log2~ID, data=sodM)
summary(sodMaov)
TukeyHSD(x=sodMaov)

p<-ggbarplot(amiA, x="ID2", y="log2", add=c("mean_sd", "point"), color="black", fill="Treatment2", palette=c("lightsteelblue4", "lightsteelblue1", "lightsteelblue3"), title = "amiA") +  
     scale_y_continuous(expand=expansion(mult=c(0.1, 0.1))) + 
     ylab(label="Log2 Transcript Expression") + 
     rremove("legend") + 
     theme(axis.text.x=element_text(angle=45, hjust=1)) + 
...
ggexport(p, filename="amiA_geom5_ggpubr_tukeyHSD_2.5x4.5_08212022.pdf", width=2.5, height=4.5)

p<-ggbarplot(tyrP, x="ID2", y="log2", add=c("mean_sd", "point"), color="black", fill="Treatment2", palette=c("lightsteelblue4", "lightsteelblue1", "lightsteelblue3"), title = "tyrP") +  
     scale_y_continuous(expand=expansion(mult=c(0.1, 0.1))) + 
     ylab(label="Log2 Transcript Expression") + 
     rremove("legend") + 
     theme(axis.text.x=element_text(angle=45, hjust=1)) + 
...
ggexport(p, filename="tyrP_geom5_ggpubr_tukeyHSD_2.5x4.5_08212022.pdf", width=2.5, height=4.5)

p<-ggbarplot(incF, x="ID2", y="log2", add=c("mean_sd", "point"), color="black", fill="Treatment2", palette=c("lightsteelblue4", "lightsteelblue1", "lightsteelblue3"), title = "incF") +  
     scale_y_continuous(expand=expansion(mult=c(0.1, 0.1))) + 
     ylab(label="Log2 Transcript Expression") + 
     rremove("legend") + 
     theme(axis.text.x=element_text(angle=45, hjust=1)) + 
     rremove("xlab") +
     theme(plot.title = element_text(face = "italic")) +
     geom_signif(comparisons=list(c("BPD16", "TRP16"), c("UTD24", "BPD16"), c("UTD24", "TRP16"), c("BPD24", "TRP24"), c("UTD24", "BPD24"), c("UTD24", "TRP24")), annotation=c("ns", "ns", "ns", "*", "**", "ns"), y_position = c(1.5, 2, 2.5, 1.5, 3, 3.5)) +
     theme(line=element_line(size=1, colour="black"), panel.border=element_rect(colour="black", fill="NA", size=1)) +
    theme(plot.title = element_text(hjust=0.5)) +
  geom_hline(yintercept=0.0)

ggexport(p, filename="incF_geom5_ggpubr_tukeyHSD_2.5x4.5_08212022.pdf", width=2.5, height=4.5)

p<-ggbarplot(incG, x="ID2", y="log2", add=c("mean_sd", "point"), color="black", fill="Treatment2", palette=c("lightsteelblue4", "lightsteelblue1", "lightsteelblue3"), title = "incG") +  
     scale_y_continuous(expand=expansion(mult=c(0.1, 0.1))) + 
     ylab(label="Log2 Transcript Expression") + 
     rremove("legend") + 
     theme(axis.text.x=element_text(angle=45, hjust=1)) + 
...
ggexport(p, filename="incG_geom5_ggpubr_tukeyHSD_2.5x4.5_08212022.pdf", width=2.5, height=4.5)

p<-ggbarplot(ispE, x="ID2", y="log2", add=c("mean_sd", "point"), color="black", fill="Treatment2", palette=c("lightsteelblue4", "lightsteelblue1", "lightsteelblue3"), title = "ispE") +  
     scale_y_continuous(expand=expansion(mult=c(0.1, 0.1))) + 
     ylab(label="Log2 Transcript Expression") + 
     rremove("legend") + 
     theme(axis.text.x=element_text(angle=45, hjust=1)) + 
     rremove("xlab") +
     theme(plot.title = element_text(face = "italic")) +
     geom_signif(comparisons=list(c("BPD16", "TRP16"), c("UTD24", "BPD16"), c("UTD24", "TRP16"), c("BPD24", "TRP24"), c("UTD24", "BPD24"), c("UTD24", "TRP24")), annotation=c("ns", "ns", "ns", "ns", "ns", "ns"), y_position = c(1.5, 2, 2.5, 1.5, 3, 3.5)) +
     theme(line=element_line(size=1, colour="black"), panel.border=element_rect(colour="black", fill="NA", size=1)) +
    theme(plot.title = element_text(hjust=0.5)) +
  geom_hline(yintercept=0.0)

ggexport(p, filename="ispE_geom5_ggpubr_tukeyHSD_2.5x4.5_08212022.pdf", width=2.5, height=4.5)

p<-ggbarplot(sodM, x="ID2", y="log2", add=c("mean_sd", "point"), color="black", fill="Treatment2", palette=c("lightsteelblue4", "lightsteelblue1", "lightsteelblue3"), title = "sodM") +  
     scale_y_continuous(expand=expansion(mult=c(0.1, 0.1))) + 
     ylab(label="Log2 Transcript Expression") + 
     rremove("legend") + 
     theme(axis.text.x=element_text(angle=45, hjust=1)) + 
...
ggexport(p, filename="sodM_geom5_ggpubr_tukeyHSD_2.5x4.5_08212022.pdf", width=2.5, height=4.5)
```
...
x<-read.csv("MIZ_geom5_unique_RTqPCR_09042022.csv")
x$Gene2<-factor(x$Gene, levels=c("amiA", "incF", "incG", "sodM", "ispE", "tyrP"))
x$ID2<-factor(x$ID, levels=c("UTD24", "MIZ24"))
...
t.test(log2 ~ ID, data = incF)
t.test(log2 ~ ID, data = incG)
t.test(log2 ~ ID, data = ispE)
t.test(log2 ~ ID, data = sodM)

p<-ggbarplot(x, x="ID2", y="log2", add=c("mean_sd", "point"), color="black", fill="ID2", palette=c("white", "chartreuse3"), facet.by="Gene2", ncol=3) +  
     scale_y_continuous(expand=expansion(mult=c(0.1, 0.1))) + 
     ylab(label="Log2 Transcript Expression") + 
     rremove("legend") + 
     theme(axis.text.x=element_text(angle=45, hjust=1)) + 
...
ggexport(p, filename="MIZ_geom5_unique_ggpubr_ttest_3x6_09042022.pdf", width=3, height=6)
```
...
#generate dds object
dds<-DESeqDataSetFromMatrix(countData=cts2, colData=coldata2, design=~batch + condition)
...
resBPD16<-lfcShrink(dds, coef=3, type='apeglm')
resBPD24<-lfcShrink(dds, coef=4, type='apeglm')
...
arld<-assay(rld)
arld <- limma::removeBatchEffect(arld, rld$batch)
assay(rld) <- arld
cbc <- assay(rld)
...
ggexport(p, filename = "Ctr_PCA_rlog_batch_6x3_06212022.pdf", width = 6, height = 3, type = pdf)
...
setwd("~/Documents/Carabeo Lab/R Code/Dual RNA-Seq/DESeq2/Ctr/FINAL/06212022")

#BPD16
BPD16_vol<-read.csv("Ctr_BPD16_DESeq2_06212022.csv")
p<-EnhancedVolcano(BPD16_vol, lab=BPD16_vol$Symbol, x='log2FoldChange', y='padj', FCcutoff = 0.58496250072, pCutoff = 5e-2, legendPosition = '', title="BPD16", subtitle="Reference = UTD24", selectLab = c("ytgA", "trpB", "trpR", "trpA", "tarp", "hct2", "hctA", "pmpG", "ompA", "omcB", "ribA", "trpC", "rplM", "incB", "amiA", "tyrP", "dapL", "atpB", "sctN", "copB", "mreB"), boxedLabels = TRUE, drawConnectors = TRUE, widthConnectors = 1.0, maxoverlapsConnectors = Inf) + theme(plot.title = element_text(hjust = 0.5)) + theme(plot.subtitle = element_text(hjust = 0.5))
ggexport(p, filename = "BPD16_Volc_7.5x7.5_06212022.pdf", width = 7.5, height = 7.5, type = pdf)

#TRP16
TRP16_vol<-read.csv("Ctr_TRP16_DESeq2_06212022.csv")
p<-EnhancedVolcano(TRP16_vol, lab=TRP16_vol$Symbol, x='log2FoldChange', y='padj', FCcutoff = 0.58496250072, pCutoff = 5e-2, legendPosition = '', title="TRP16", subtitle="Reference = UTD24", selectLab = c("ytgA", "trpB", "trpR", "trpA", "tarp", "hct2", "hctA", "pmpG", "ompA", "omcB", "ribA", "trpC", "rplM", "incB", "amiA", "tyrP", "dapL", "atpB", "sctN", "copB", "mreB"), boxedLabels = TRUE, drawConnectors = TRUE, widthConnectors = 1.0, maxoverlapsConnectors = Inf) + theme(plot.title = element_text(hjust = 0.5)) + theme(plot.subtitle = element_text(hjust = 0.5))
ggexport(p, filename = "TRP16_Volc_7.5x7.5_06212022.pdf", width = 7.5, height = 7.5, type = pdf)

#BPD24
BPD24_vol<-read.csv("Ctr_BPD24_DESeq2_06212022.csv")
p<-EnhancedVolcano(BPD24_vol, lab=BPD24_vol$Symbol, x='log2FoldChange', y='padj', FCcutoff = 0.58496250072, pCutoff = 5e-2, legendPosition = '', title="BPD24", subtitle="Reference = UTD24", selectLab = c("ytgA", "trpB", "trpR", "trpA", "tarp", "hct2", "hctA", "pmpG", "ompA", "omcB", "ribA", "trpC", "rplM", "incB", "amiA", "tyrP", "dapL", "atpB", "sctN", "copB", "mreB"), boxedLabels = TRUE, drawConnectors = TRUE, widthConnectors = 1.0, maxoverlapsConnectors = Inf) + theme(plot.title = element_text(hjust = 0.5)) + theme(plot.subtitle = element_text(hjust = 0.5))
ggexport(p, filename = "BPD24_Volc_7.5x7.5_06212022.pdf", width = 7.5, height = 7.5, type = pdf)

#TRP24
TRP24_vol<-read.csv("Ctr_TRP24_DESeq2_06212022.csv")
p<-EnhancedVolcano(TRP24_vol, lab=TRP24_vol$Symbol, x='log2FoldChange', y='padj', FCcutoff = 0.58496250072, pCutoff = 5e-2, legendPosition = '', title="TRP24", subtitle="Reference = UTD24", selectLab = c("ytgA", "trpB", "trpR", "trpA", "tarp", "hct2", "hctA", "pmpG", "ompA", "omcB", "ribA", "trpC", "rplM", "incB", "amiA", "tyrP", "dapL", "atpB", "sctN", "copB", "mreB"), boxedLabels = TRUE, drawConnectors = TRUE, widthConnectors = 1.0, maxoverlapsConnectors = Inf) + theme(plot.title = element_text(hjust = 0.5)) + theme(plot.subtitle = element_text(hjust = 0.5))
ggexport(p, filename = "TRP24_Volc_7.5x7.5_06212022.pdf", width = 7.5, height = 7.5, type = pdf)
...
write.csv(as.data.frame(BPD16k), file="Ctr_BPD16_gseKEGG_06212022.csv")

#dotplot of enriched KEGG terms
p<-dotplot(BPD16k, showCategory = 5, orderBy = "x") +
  ggtitle("BPD16 KEGG") +
  theme(plot.title = element_text(hjust = 0.5)) +
  scale_x_continuous(limits=c(0.4, 1.0))
ggexport(p, filename = "BPD16_dot_gseKEGG_4.5x4_06212022.pdf", width = 4.5, height = 4, type = pdf)

#ridgeplot of enriched KEGG terms by gene expression
p<-ridgeplot(BPD16k, showCategory = 5) +
  ggtitle("BPD16 KEGG") +
  theme(plot.title = element_text(hjust = 0.5)) +
  xlab("Log2 Fold Change")
ggexport(p, filename = "BPD16_ridge_gseKEGG_5.5x4_06212022.pdf", width = 5.5, height = 4, type = pdf)

#plot pathview for enriched KEGG terms
BPD16l<-read.csv("Ctr_BPD16_gseKEGG_06212022.csv")
setwd("~/Documents/Carabeo Lab/R Code/Dual RNA-Seq/DESeq2/Ctr/FINAL/06212022/clusterProfiler/BPD16")
...
#TRP16
setwd("~/Documents/Carabeo Lab/R Code/Dual RNA-Seq/DESeq2/Ctr/FINAL/06212022")
TRP16<-read.csv("Ctr_TRP16_DESeq2_06212022.csv")
...
write.csv(as.data.frame(TRP16k), file="Ctr_TRP16_gseKEGG_06212022.csv")

p<-dotplot(TRP16k, showCategory = 5, orderBy = "x") +
  ggtitle("TRP16 KEGG") +
  theme(plot.title = element_text(hjust = 0.5)) +
  scale_x_continuous(limits=c(0.2, 1.0))
ggexport(p, filename = "TRP16_dot_gseKEGG_4.5x4_06212022.pdf", width = 4.5, height = 4, type = pdf)

p<-ridgeplot(TRP16k, showCategory = 5) +
  ggtitle("TRP16 KEGG") +
  theme(plot.title = element_text(hjust = 0.5)) +
  xlab("Log2 Fold Change")
ggexport(p, filename = "TRP16_ridge_gseKEGG_5.5x4_06212022.pdf", width = 5.5, height = 4, type = pdf)

TRP16l<-read.csv("Ctr_TRP16_gseKEGG_06212022.csv")
setwd("~/Documents/Carabeo Lab/R Code/Dual RNA-Seq/DESeq2/Ctr/FINAL/06212022/clusterProfiler/TRP16")
...
#BPD24
setwd("~/Documents/Carabeo Lab/R Code/Dual RNA-Seq/DESeq2/Ctr/FINAL/06212022")
BPD24<-read.csv("Ctr_BPD24_DESeq2_06212022.csv")
...
write.csv(as.data.frame(BPD24k), file="Ctr_BPD24_gseKEGG_06212022.csv")

p<-dotplot(BPD24k, showCategory = 5, orderBy = "x") +
  ggtitle("BPD24 KEGG") +
  theme(plot.title = element_text(hjust = 0.5)) +
  scale_x_continuous(limits=c(0.4, 1.0))
ggexport(p, filename = "BPD24_dot_gseKEGG_4.5x4_06212022.pdf", width = 4.5, height = 4, type = pdf)

p<-ridgeplot(BPD24k, showCategory = 5) +
  ggtitle("BPD24 KEGG") +
  theme(plot.title = element_text(hjust = 0.5)) +
  xlab("Log2 Fold Change")
ggexport(p, filename = "BPD24_ridge_gseKEGG_5.5x4_06212022.pdf", width = 5.5, height = 4, type = pdf)

BPD24l<-read.csv("Ctr_BPD24_gseKEGG_06212022.csv")
setwd("~/Documents/Carabeo Lab/R Code/Dual RNA-Seq/DESeq2/Ctr/FINAL/06212022/clusterProfiler/BPD24")
...
TRP24<-read.csv("Ctr_TRP24_DESeq2_06212022.csv")
TRP24list<-TRP24[,3]
...
write.csv(as.data.frame(TRP24k), file="Ctr_TRP24_gseKEGG_06212022.csv")

p<-dotplot(TRP24k, showCategory = 5, orderBy = "x") +
  ggtitle("TRP24 KEGG") +
  theme(plot.title = element_text(hjust = 0.5)) +
  scale_x_continuous(limits=c(0.2, 1.0))
ggexport(p, filename = "TRP24_dot_gseKEGG_4.5x4_06212022.pdf", width = 4.5, height = 4, type = pdf)

p<-ridgeplot(TRP24k, showCategory = 5) +
  ggtitle("TRP24 KEGG") +
  theme(plot.title = element_text(hjust = 0.5)) +
  xlab("Log2 Fold Change")
ggexport(p, filename = "TRP24_ridge_gseKEGG_5.5x4_06212022.pdf", width = 5.5, height = 4, type = pdf)

TRP24l<-read.csv("Ctr_TRP24_gseKEGG_06212022.csv")
setwd("~/Documents/Carabeo Lab/R Code/Dual RNA-Seq/DESeq2/Ctr/FINAL/06212022/clusterProfiler/TRP24")
...
#to generate Venn diagram plots, I used the package ggVennDiagram
...
p<-ggVennDiagram(y, edge_lty = "blank") + 
  ggtitle("C. trachomatis\n Down-regulated") + 
  theme(plot.title=element_text(hjust=0.5)) +
  scale_fill_gradient(low = "white", high = "red")
ggexport(p, filename = "Ctr_DOWN_Venn_Volc_8.5x6.5_06212022.pdf", width = 8.5, height = 6.5, type = pdf)
...
p<-ggVennDiagram(x, edge_lty = "blank") +
  ggtitle("C. trachomatis\n Up-regulated") +
  theme(plot.title=element_text(hjust=0.5)) +
  scale_fill_gradient(low = "white", high = "red")
ggexport(p, filename = "Ctr_UP_Venn_Volc_8.5x6.5_06212022.pdf", width = 8.5, height = 6.5, type = pdf)
...
```{r}
#DESeq2 - HeLa 
setwd("~/Documents/Carabeo Lab/R Code/Dual RNA-Seq/DESeq2/HeLa/Final/06212022")
...
resBPD16<-lfcShrink(dds, coef=3, type='apeglm')
resBPD24<-lfcShrink(dds, coef=4, type='apeglm')
resTRP16<-lfcShrink(dds, coef=6, type='apeglm')
resTRP24<-lfcShrink(dds, coef=7, type='apeglm')
#Note that Mock v UTD will show DE relative to UTD24!
resMock<-lfcShrink(dds, coef=5, type='apeglm')
#for ref=Mock
resUTD24<-lfcShrink(dds, coef=7, type='apeglm')
...
write.csv(as.data.frame(resUTD24), file="HeLa_UTD24_DESeq2_lfc_04042021.csv")
...
ggexport(p, filename = "HeLa_PCA_rlog_batch_4x4_06212022.pdf", width = 4, height = 4, type = pdf)
...
UTD24_vol$padj2 <- ifelse(UTD24_vol$padj < 1e-30, 1e-30, UTD24_vol$padj)
p<-EnhancedVolcano(UTD24_vol, lab=UTD24_vol$symbol, x='log2FoldChange', y='padj', pCutoff = 5e-2, legendPosition = '', title="UTD24", subtitle="Reference = Mock", selectLab = c("IDO1", "IL1B", "CXCL11", "OASL", "IL6", "EGF", "CCL5", "JUN", "ITGB1", "SDHB", "CYC1", "ND4", "COX1", "ACTB", "TLN1", "ZYX", "VCL", "MMP1", "RPL34", "RPS14", "NCBP1", "SRSF1", "PPAT", "GART", "GMPS", "IMPDH1", "AMPD1", "NME1", "ATIC"), boxedLabels = TRUE, drawConnectors = TRUE, widthConnectors = 1.0, maxoverlapsConnectors = Inf) + theme(plot.title = element_text(hjust = 0.5)) + theme(plot.subtitle = element_text(hjust = 0.5)) + coord_cartesian(ylim = c(-2.5, 60))
ggexport(p, filename = "UTD24_HeLa_Volc_7.5x7.5_06212022.pdf", width = 7.5, height = 7.5, type = pdf)

#BPD16
BPD16_vol<-read.csv("HeLa_BPD16_DESeq2_lfc_04042021.csv")
BPD16_vol$padj2 <- ifelse(BPD16_vol$padj < 1e-30, 1e-30, BPD16_vol$padj)
p<-EnhancedVolcano(BPD16_vol, lab=BPD16_vol$symbol, x='log2FoldChange', y='padj2', pCutoff = 5e-2, legendPosition = '', title="BPD16", subtitle="Reference = UTD24", selectLab = c("IDO1", "IL1B", "CXCL11", "OASL", "IL6", "EGF", "CCL5", "JUN", "ITGB1", "SDHB", "CYC1", "ND4", "COX1", "ACTB", "TLN1", "ZYX", "VCL", "MMP1", "RPL34", "RPS14", "NCBP1", "SRSF1", "PPAT", "GART", "GMPS", "IMPDH1", "AMPD1", "NME1", "ATIC", "DUOX2", "CSF3", "MAP10", "H1-3", "H2BC14"), boxedLabels = TRUE, drawConnectors = TRUE, widthConnectors = 1.0, maxoverlapsConnectors = Inf) +
    theme(plot.title = element_text(hjust = 0.5)) +
    theme(plot.subtitle = element_text(hjust = 0.5)) +
    coord_cartesian(ylim = c(-2, 30))
ggexport(p, filename = "BPD16_HeLa_Volc_7.5x7.5_06212022.pdf", width = 7.5, height = 7.5, type = pdf)

#adjust ylim
p<-EnhancedVolcano(BPD16_vol, lab=BPD16_vol$symbol, x='log2FoldChange', y='padj', pCutoff = 5e-2, legendPosition = '', title="BPD16", subtitle="Reference = UTD24", selectLab = c("IDO1", "IL1B", "CXCL11", "OASL", "IL6", "EGF", "CCL5", "JUN", "ITGB1", "SDHB", "CYC1", "ND4", "COX1", "ACTB", "TLN1", "ZYX", "VCL", "MMP1", "RPL34", "RPS14", "NCBP1", "SRSF1", "PPAT", "GART", "GMPS", "IMPDH1", "AMPD1", "NME1", "ATIC", "DUOX2", "CSF3"), boxedLabels = TRUE, drawConnectors = TRUE, widthConnectors = 1.0, maxoverlapsConnectors = Inf) + theme(plot.title = element_text(hjust = 0.5)) + theme(plot.subtitle = element_text(hjust = 0.5)) + coord_cartesian(ylim = c(-2.5, 60))
ggexport(p, filename = "BPD16_HeLa_Volc_7.5x7.5_06212022.pdf", width = 7.5, height = 7.5, type = pdf)
...
p<-EnhancedVolcano(TRP16_vol, lab=TRP16_vol$symbol, x='log2FoldChange', y='padj2', pCutoff = 5e-2, legendPosition = '', title="TRP16", subtitle="Reference = UTD24", selectLab = c("IDO1", "IL1B", "CXCL11", "OASL", "IL6", "EGF", "CCL5", "JUN", "ITGB1", "SDHB", "CYC1", "ND4", "COX1", "ACTB", "TLN1", "ZYX", "VCL", "MMP1", "RPL34", "RPS14", "NCBP1", "SRSF1", "PPAT", "GART", "GMPS", "IMPDH1", "AMPD1", "NME1", "ATIC", "APOBEC3A", "NOS2", "IFNL2", "MTUS2", "FGFBP1"), boxedLabels = TRUE, drawConnectors = TRUE, widthConnectors = 1.0, maxoverlapsConnectors = Inf) +
    theme(plot.title = element_text(hjust = 0.5)) +
    theme(plot.subtitle = element_text(hjust = 0.5)) +
    coord_cartesian(ylim = c(-2, 30))
ggexport(p, filename = "TRP16_HeLa_Volc_7.5x7.5_06212022.pdf", width = 7.5, height = 7.5, type = pdf)

#BPD24
BPD24_vol<-read.csv("HeLa_BPD24_DESeq2_lfc_04042021.csv")
BPD24_vol$padj2 <- ifelse(BPD24_vol$padj < 1e-50, 1e-50, BPD24_vol$padj)
p<-EnhancedVolcano(BPD24_vol, lab=BPD24_vol$symbol, x='log2FoldChange', y='padj2', pCutoff = 5e-2, legendPosition = '', title="BPD24", subtitle="Reference = UTD24", selectLab = c("IDO1", "IL1B", "CXCL11", "OASL", "IL6", "EGF", "CCL5", "JUN", "ITGB1", "SDHB", "CYC1", "ND4", "COX1", "ACTB", "TLN1", "ZYX", "VCL", "MMP1", "RPL34", "RPS14", "NCBP1", "SRSF1", "PPAT", "GART", "GMPS", "IMPDH1", "AMPD1", "NME1", "ATIC", "DUOX2", "CSF3", "MAP10", "H1-3", "H2BC14"), boxedLabels = TRUE, drawConnectors = TRUE, widthConnectors = 1.0, maxoverlapsConnectors = Inf) +
    theme(plot.title = element_text(hjust = 0.5)) +
    theme(plot.subtitle = element_text(hjust = 0.5)) +
    coord_cartesian(ylim = c(-2, 50))
ggexport(p, filename = "BPD24_HeLa_Volc_7.5x7.5_06212022.pdf", width = 7.5, height = 7.5, type = pdf)

#adjust ylim
p<-EnhancedVolcano(BPD24_vol, lab=BPD24_vol$symbol, x='log2FoldChange', y='padj', pCutoff = 5e-2, legendPosition = '', title="BPD24", subtitle="Reference = UTD24", selectLab = c("IDO1", "IL1B", "CXCL11", "OASL", "IL6", "EGF", "CCL5", "JUN", "ITGB1", "SDHB", "CYC1", "ND4", "COX1", "ACTB", "TLN1", "ZYX", "VCL", "MMP1", "RPL34", "RPS14", "NCBP1", "SRSF1", "PPAT", "GART", "GMPS", "IMPDH1", "AMPD1", "NME1", "ATIC", "DUOX2", "CSF3"), boxedLabels = TRUE, drawConnectors = TRUE, widthConnectors = 1.0, maxoverlapsConnectors = Inf) + theme(plot.title = element_text(hjust = 0.5)) + theme(plot.subtitle = element_text(hjust = 0.5)) + coord_cartesian(ylim = c(-2.5, 100)) 
ggexport(p, filename = "BPD24_HeLa_Volc_7.5x7.5_06212022.pdf", width = 7.5, height = 7.5, type = pdf)

#TRP24
TRP24_vol<-read.csv("HeLa_TRP24_DESeq2_lfc_04042021.csv")
TRP24_vol$padj2 <- ifelse(TRP24_vol$padj < 1e-30, 1e-30, TRP24_vol$padj)
p<-EnhancedVolcano(TRP24_vol, lab=TRP24_vol$symbol, x='log2FoldChange', y='padj2', pCutoff = 5e-2, legendPosition = '', title="TRP24", subtitle="Reference = UTD24", selectLab = c("IDO1", "IL1B", "CXCL11", "OASL", "IL6", "EGF", "CCL5", "JUN", "ITGB1", "SDHB", "CYC1", "ND4", "COX1", "ACTB", "TLN1", "ZYX", "VCL", "MMP1", "RPL34", "RPS14", "NCBP1", "SRSF1", "PPAT", "GART", "GMPS", "IMPDH1", "AMPD1", "NME1", "ATIC", "APOBEC3A", "NOS2", "IFNL2", "MTUS2", "FGFBP1"), boxedLabels = TRUE, drawConnectors = TRUE, widthConnectors = 1.0, maxoverlapsConnectors = Inf) +
    theme(plot.title = element_text(hjust = 0.5)) +
    theme(plot.subtitle = element_text(hjust = 0.5)) +
    coord_cartesian(ylim = c(-2, 30))
ggexport(p, filename = "TRP24_HeLa_Volc_7.5x7.5_06212022.pdf", width = 7.5, height = 7.5, type = pdf)
...
p<-dotplot(UTD24k, showCategory=15, orderBy = "x", split=".sign") +
  ggtitle("UTD24 KEGG") +
  theme(plot.title = element_text(hjust = 0.5)) +
  facet_grid(.~.sign) +
  scale_x_continuous(limits=c(0.1, 0.6))
ggexport(p, filename = "UTD24_dot_gseKEGG_7.5x10_06212022.pdf", width = 7.5, height = 10, type = pdf)
...
p<-dotplot(mUTD24k, showCategory = 15, orderBy = "x") +
  ggtitle("UTD24 KEGG") +
  theme(plot.title = element_text(hjust = 0.5)) +
  scale_x_continuous(limits=c(0.1, 0.6))
ggexport(p, filename = "UTD24_dot_gseKEGG_6x6_06212022.pdf", width = 6, height = 6, type = pdf)
...
p<-emapplot(pUTD24k, showCategory = 15)
ggexport(p, filename = "UTD24_emap_gseKEGG_7.5x7.5_06212022.pdf", width = 7.5, height = 7.5, type = pdf)

#ridgeline plot of top 15 significant KEGG terms
p<-ridgeplot(UTD24k, showCategory = 15) +
  ggtitle("UTD24 KEGG") +
  theme(plot.title = element_text(hjust = 0.5)) +
  xlab("Log2 Fold Change")
ggexport(p, filename = "UTD24_ridge_gseKEGG_6x6_06212022.pdf", width = 6, height = 6, type = pdf)

#export pathview files
UTD24l<-read.csv("HeLa_UTD24_gseKEGG_04142021.csv")
setwd("~/Documents/Carabeo Lab/R Code/Dual RNA-Seq/DESeq2/HeLa/Final/06212022/clusterProfiler/UTD24")
...
#repeat above for each condition
#BPD16
setwd("~/Documents/Carabeo Lab/R Code/Dual RNA-Seq/DESeq2/HeLa/Final/06212022")
...
p<-dotplot(BPD16k, showCategory=15, orderBy = "x", split=".sign") +
  ggtitle("BPD16 KEGG") +
  theme(plot.title = element_text(hjust = 0.5)) +
  facet_grid(.~.sign) +
  scale_x_continuous(limits=c(0.1, 0.8))
ggexport(p, filename = "BPD16_dot_gseKEGG_7.5x10_06212022.pdf", width = 7.5, height = 10, type = pdf)

mBPD16k <- mutate(BPD16k) %>% arrange(desc(NES))
write.csv(as.data.frame(mBPD16k), file="HeLa_BPD16_arrangeNES_gseKEGG_04142021.csv")
p<-dotplot(mBPD16k, showCategory = 15, orderBy = "x") +
  ggtitle("BPD16 KEGG") +
  theme(plot.title = element_text(hjust = 0.5)) +
  scale_x_continuous(limits=c(0.1, 0.7))
ggexport(p, filename = "BPD16_dot_gseKEGG_6x6_06212022.pdf", width = 6, height = 6, type = pdf)

pBPD16k <- pairwise_termsim(mBPD16k, method = "JC", semData = NULL, showCategory = 400)
p<-emapplot(pBPD16k, showCategory = 15)
ggexport(p, filename = "BPD16_emap_gseKEGG_7.5x7.5_06212022.pdf", width = 7.5, height = 7.5, type = pdf)

p<-ridgeplot(BPD16k, showCategory = 15) +
  ggtitle("BPD16 KEGG") +
  theme(plot.title = element_text(hjust = 0.5)) +
  xlab("Log2 Fold Change")
ggexport(p, filename = "BPD16_ridge_gseKEGG_6x6_06212022.pdf", width = 6, height = 6, type = pdf)

BPD16l<-read.csv("HeLa_BPD16_gseKEGG_04142021.csv")
setwd("~/Documents/Carabeo Lab/R Code/Dual RNA-Seq/DESeq2/HeLa/Final/06212022/clusterProfiler/BPD16")
...
p<-dotplot(mTRP16k, showCategory = 15, orderBy = "x") +
  ggtitle("TRP16 KEGG") +
  theme(plot.title = element_text(hjust = 0.5)) +
  scale_x_continuous(limits=c(0.1, 0.4))
ggexport(p, filename = "TRP16_dot_gseKEGG_6x6_06212022.pdf", width = 6, height = 6, type = pdf)

pTRP16k <- pairwise_termsim(mTRP16k, method = "JC", semData = NULL, showCategory = 400)
p<-emapplot(pTRP16k, showCategory = 15)
ggexport(p, filename = "TRP16_emap_gseKEGG_7.5x7.5_06212022.pdf", width = 7.5, height = 7.5, type = pdf)

p<-ridgeplot(TRP16k, showCategory = 15) +
  ggtitle("TRP16 KEGG") +
  theme(plot.title = element_text(hjust = 0.5)) +
  xlab("Log2 Fold Change")
ggexport(p, filename = "TRP16_ridge_gseKEGG_6x6_06212022.pdf", width = 6, height = 6, type = pdf)

TRP16l<-read.csv("HeLa_TRP16_gseKEGG_04142021.csv")
setwd("~/Documents/Carabeo Lab/R Code/Dual RNA-Seq/DESeq2/HeLa/Final/06212022/clusterProfiler/TRP16")
...
p<-dotplot(BPD24k, showCategory=15, orderBy = "x", split=".sign") +
  ggtitle("BPD24 KEGG") +
  theme(plot.title = element_text(hjust = 0.5)) +
  facet_grid(.~.sign) +
  scale_x_continuous(limits=c(0.1, 0.7))
ggexport(p, filename = "BPD24_dot_gseKEGG_7.5x10_06212022.pdf", width = 7.5, height = 10, type = pdf)

mBPD24k <- mutate(BPD24k) %>% arrange(desc(NES))
write.csv(as.data.frame(mBPD24k), file="HeLa_BPD24_arrangeNES_gseKEGG_04142021.csv")

p<-dotplot(mBPD24k, showCategory = 15, orderBy = "x") +
  ggtitle("BPD24 KEGG") +
  theme(plot.title = element_text(hjust = 0.5)) +
  scale_x_continuous(limits=c(0.1, 0.7))
ggexport(p, filename = "BPD24_dot_gseKEGG_6x6_06212022.pdf", width = 6, height = 6, type = pdf)

pBPD24k <- pairwise_termsim(mBPD24k, method = "JC", semData = NULL, showCategory = 400)
p<-emapplot(pBPD24k, showCategory = 15)
ggexport(p, filename = "BPD24_emap_gseKEGG_7.5x7.5_06212022.pdf", width = 7.5, height = 7.5, type = pdf)

p<-ridgeplot(BPD24k, showCategory = 15) +
  ggtitle("BPD24 KEGG") +
  theme(plot.title = element_text(hjust = 0.5)) +
  xlab("Log2 Fold Change")
ggexport(p, filename = "BPD24_ridge_gseKEGG_6x6_06212022.pdf", width = 6, height = 6, type = pdf)

#ascending
mBPD24k <- mutate(BPD24k) %>% arrange(NES)
write.csv(as.data.frame(mBPD24k), file="HeLa_BPD24_arrangeNES_ascend_gseKEGG_04142021.csv")

p<-dotplot(mBPD24k, showCategory = 12, orderBy = "x") +
  ggtitle("BPD24 KEGG - Suppressed") +
  theme(plot.title = element_text(hjust = 0.5)) +
  scale_x_continuous(limits=c(0.1, 0.7))
ggexport(p, filename = "BPD24_dot_ascend_gseKEGG_6x6_06212022.pdf", width = 6, height = 6, type = pdf)

BPD24l<-read.csv("HeLa_BPD24_gseKEGG_04142021.csv")
setwd("~/Documents/Carabeo Lab/R Code/Dual RNA-Seq/DESeq2/HeLa/Final/06212022/clusterProfiler/BPD24")
...
p<-dotplot(mTRP24k, showCategory = 15, orderBy = "x") +
  ggtitle("TRP24 KEGG") +
  theme(plot.title = element_text(hjust = 0.5)) +
  scale_x_continuous(limits=c(0.1, 0.5))
ggexport(p, filename = "TRP24_dot_gseKEGG_6x6_06212022.pdf", width = 6, height = 6, type = pdf)

pTRP24k <- pairwise_termsim(mTRP24k, method = "JC", semData = NULL, showCategory = 400)
p<-emapplot(pTRP24k, showCategory = 15)
ggexport(p, filename = "TRP24_emap_gseKEGG_7.5x7.5_06212022.pdf", width = 7.5, height = 7.5, type = pdf)

p<-ridgeplot(TRP24k, showCategory = 15) +
  ggtitle("TRP24 KEGG") +
  theme(plot.title = element_text(hjust = 0.5)) +
  xlab("Log2 Fold Change")
ggexport(p, filename = "TRP24_ridge_gseKEGG_6x6_06212022.pdf", width = 6, height = 6, type = pdf)

TRP24l<-read.csv("HeLa_TRP24_gseKEGG_04142021.csv")
setwd("~/Documents/Carabeo Lab/R Code/Dual RNA-Seq/DESeq2/HeLa/Final/06212022/clusterProfiler/TRP24")
